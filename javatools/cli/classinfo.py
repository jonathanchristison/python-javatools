...
from ..opcodes import get_opname_by_code
...
def print_method(options, method):
    ...
    if options.disassemble and code:
        ...
        for line in code.disassemble():
            opname = get_opname_by_code(line[1])
            args = line[2]
            if args:
...
def ifonly(data, key, val):
    """
    utility function to set data[key] to val, but only if val has a
    truthy value
    """
...
def create_optparser():
    parser = OptionParser("%prog [OPTIONS] CLASSFILE [...]")
...
def main(args):
    """
    Command-line entry point for the `classinfo` utility script.
    """
